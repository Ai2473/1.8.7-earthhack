...
public class ModuleComponent extends Component {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (getClickGui().get().showBind.getValue() && !getModule().getBind().toString().equalsIgnoreCase("none")) {
            ...
            Managers.TEXT.drawStringWithShadow(disString, (offset - (Managers.TEXT.getStringWidth(disString) >> 1)) * 2 - 12, (getFinishedY() + getHeight() / 1.5f - (Managers.TEXT.getStringHeightI() >> 1)) * 2.0f, getModule().isEnabled() ? getColor().get().getOnModule().brighter().getRGB() : getColor().get().getOffModule().brighter().getRGB());
